...
All notable changes to `@absolutejs/queue`.
...
- **Republish the verified worker kind-routing fix after npm accepted 0.8.3 but left it unavailable and unlisted in staging** (`createQueueWorker`)

## 0.8.3 — 2026-09-16

### Fixed

- **Keep jobs pending for workers with matching handlers using atomic kind-filtered claims and an optional strict adapter capability gate** (`createQueueWorker`, `ClaimDueOptions`, `JobStore`, `CreateQueueWorkerOptions`, `createInMemoryJobStore`)

## 0.8.2 — 2026-09-16

### Added

- **Add atomic claim-fenced completion and failure, unique in-memory claim tokens and a strict worker capability gate so expired attempts cannot overwrite newer jobs** (`JobStore`, `JobContext`, `createQueueWorker`, `createInMemoryJobStore`, `CreateQueueWorkerOptions`, `Job`)

### Fixed

- **Count expired worker claims toward maxAttempts so repeated process death cannot retry a job indefinitely** (`createInMemoryJobStore`)
